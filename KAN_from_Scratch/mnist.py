...
x_train, x_val = (x_train / 255.0 - 0.5).astype("float32"), (
    x_val / 255.0 - 0.5
).astype(
    "float32"
)  # Normalize
...
for epoch in range(epochs):
    ...
    print(
        f"Epoch {epoch + 1}, Train Loss: {train_loss.result():.4f}, Validation Loss: {val_loss.result():.4f}, Validation Accuracy: {val_accuracy.result():.4f}"
    )
